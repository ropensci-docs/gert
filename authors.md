# Authors and Citation

## Authors

- **[Jeroen Ooms](https://github.com/jeroen)**. Author, maintainer.
  [](https://orcid.org/0000-0002-4035-0289)

- **[Maëlle Salmon](https://masalmon.eu)**. Author.
  [](https://orcid.org/0000-0002-2815-0399)

- **Jennifer Bryan**. Contributor.
  [](https://orcid.org/0000-0002-6983-2759)

## Citation

Source:
[`DESCRIPTION`](https://github.com/r-lib/gert/blob/HEAD/DESCRIPTION)

Ooms J, Salmon M (2026). *gert: Simple Git Client for R*. R package
version 2.5.0, <https://docs.ropensci.org/gert/>.

    @Manual{,
      title = {gert: Simple Git Client for R},
      author = {Jeroen Ooms and Maëlle Salmon},
      year = {2026},
      note = {R package version 2.5.0},
      url = {https://docs.ropensci.org/gert/},
    }
